...
. Deploy averageCadence.ctf to a server instance
. Develop a client application to access the functions.

1. Deploy averageCadence.ctf

1.1 Prerequisites to Deployment
Before you can deploy averageCadence.ctf you need a server instance to host it. The 
server instance must be configured to have access to MATLAB runtime 8.4.
* If you do not have a server instance created see [Server 
  Creation](http://mathworks.com/help/mps/creating_a_server.html).
* If you do not have version 8.4 of the MATLAB runtime installed download the installer 
...
* If you need to configure the server instance to use the version 8.4 of the MATLAB 
  runtime see [Configuration File 
  Customization](http://www.mathworks.com/help/mps/customize-the-configuration-file.html).

1.2 averageCadence.ctf Deployment
To deploy averageCadence.ctf copy it into the server instance's `auto_deploy` folder. The 
